...
def final_validation(doc):
	...
	""", {"lc_no": doc.lc_no, "name": doc.name or ""}, as_dict=True)
	...
	if doc.loan_amount > max_lc_amount - lc_paid_amount - imp_l_amount - u_lc_amount:
		...
		msg += f"<b>Entered Amount:</b> {doc.loan_amount:,.2f}<br>"

		frappe.throw(msg)

	if doc.loan_amount == 0:
		frappe.throw("⚠️ <b>Validation Error:</b> Please enter a valid Loan Amount. It cannot be zero.")
