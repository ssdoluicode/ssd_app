...
frappe.ui.form.on("CC Received", {
    ...
    refresh(frm) {
        ...
        // Ensure balance calculation on refresh
        if (!frm.is_new()) {
            recalc_balance(frm);
        }
    ...
    after_save(frm) {
        ...
        if (frappe.route_options?.redirect_after_save) {

            let report_name = frappe.route_options.redirect_after_save;
            frappe.route_options = null;
            frappe.set_route("query-report", report_name).then(() => {
...
function set_amount_from_previous_balance(frm, cdt, cdn) {
    ...
    if (idx === 0) {
        if (!row.amount) {
            row.amount = flt(frm.doc.amount_usd || 0);
        }
    } else {
...
function recalc_balance(frm) {
    ...
    if (Math.abs(running_balance) < 0.01) {
        ...
        frm.set_df_property(
            ...
            "description",
            `<b style="color:red">
                Remaining Balance: ${running_balance.toFixed(2)}
            </b>`
        );
    }
}
